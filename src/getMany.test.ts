...
import { createStore, getMany, setMany } from './index.js'
...
it('does not throw if the keys are not set', async () => {
  ...
  // Test
  await expect(getMany(db, ['key-1'])).resolves.toMatchInlineSnapshot(`
    [
      undefined,
    ]
  `)
...
it('returns the values for the given keys', async () => {
  ...
  // Test
  expect(await getMany(db, ['key-1', 'key-3'])).toMatchInlineSnapshot(`
    [
      {
        "message": "hello 1",
      },
